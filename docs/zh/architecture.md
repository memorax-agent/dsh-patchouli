...
尚未实现。Bundle 内的 TypeScript 存储客户端已经暴露后端协议中的控制、CRUD、实体检索、
...
远程插件可以直接调用外部 API，本地插件可以使用存储客户端。存储 CRUD 类型
...
解析并验证工作区边界，然后通过 `ctx.patchouliStorage.uploadArtifact` 传输。高层
协调服务、存储客户端、Attachment 或 FS 服务缺失时 Cordis Fiber 保持 Pending。
...
`dsh-patchouli/cursor-store` 注册 `ctx.patchouliCursors`。`bind` 绑定
...
`dsh-patchouli/storage` 注册 `ctx.patchouliStorage`，连接既有 Daemon，并可在 Endpoint
不可用时启动一个。默认 Bundle 已包含该客户端；全部使用远程 MemoryPlugin 的部署可
禁用对应 Bundle 条目。
...
Harness-neutral 事务后端、本地/远程 Provider、Bundle 内 TypeScript 存储客户端、DSH
