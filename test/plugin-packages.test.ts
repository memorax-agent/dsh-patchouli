...
test('CRUD test plugin declares an isolated third-party boundary', () => {
  ...
  assert.deepEqual(crudTestPlugin.inject, ['patchouliMemory', 'patchouli'])
})
...
test('session indexer declares its package boundary', () => {
  ...
  assert.deepEqual(sessionIndexer.inject, ['patchouliMemory', 'sessionQuery'])
...
test('workspace indexer declares its package boundary', () => {
  ...
  assert.deepEqual(workspaceIndexer.inject, ['patchouliMemory', 'workspaceRegistry', 'fs'])
